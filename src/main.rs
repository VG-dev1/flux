...
use ratatui::{
    prelude::*,
    widgets::{Block, Borders, Cell, Clear, Paragraph, Row, Table, TableState, Sparkline},
};
...
fn draw_resource_graphs(frame: &mut Frame, history: &ResourceHistory) {
    ...
    let cpu_line = Sparkline::default()
        .block(
            Block::default()
                .title(Line::from(format!(" CPU USAGE ({}%) ", &history.cpu_data.last().unwrap_or(&0))).style(Style::default().fg(orange)))
                .borders(Borders::ALL)
